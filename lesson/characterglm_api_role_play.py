...
with st.container():
    ...
    with col1:
        st.text_input(label="请输入文本1", key="text1", on_change=lambda : st.session_state["meta"].update(text1=st.session_state["text1"]), help="系统会根据该文本自动生成人物1的人设，不可为空")
        gen_role1 = st.button(meta_labels["gen_role1"], key="gen_role1")
        st.text_area(label="角色名", value="角色", key="bot_name", on_change=lambda : st.session_state["meta"].update(bot_name=st.session_state["bot_name"]), help="角色的名字，默认为用户")
        st.text_area(label="角色人设", value="", key="bot_info", on_change=lambda : st.session_state["meta"].update(bot_info=st.session_state["bot_info"]), help="用户的详细人设信息，可以为空")
        
    with col2:
        st.text_input(label="请输入文本2", key="text2", on_change=lambda : st.session_state["meta"].update(text2=st.session_state["text2"]), help="系统会根据该文本自动生成人物2的人设，不可为空")
        gen_role2 = st.button(meta_labels["gen_role2"], key="gen_role2")
        st.text_area(label="用户名", value="用户", key="user_name", on_change=lambda : st.session_state["meta"].update(user_name=st.session_state["user_name"]), help="用户的名字，默认为用户")
...
def gen_role_character_profile1(text):
    ...
    st.session_state["meta"]["bot_name"] = role
    st.session_state["meta"]["bot_info"] = character_profile
