...
async def get_characterglm_response_via_https_async(messages: TextMsgList, meta: CharacterMeta) -> Generator[str, None, None]:
    """ 通过http调用characterglm """
    # Reference: https://open.bigmodel.cn/dev/api#characterglm
    verify_api_key_not_empty()
...
def get_chatglm_response_via_sdk2(messages: TextMsgList) -> Generator[str, None, None]:
    """ 通过sdk调用chatglm """
    # reference: https://open.bigmodel.cn/dev/api#glm-3-turbo  `GLM-3-Turbo`相关内容
    # 需要安装新版zhipuai
    from zhipuai import ZhipuAI
...
def generate_character_profile(text: str)-> Generator[str, None, None]:
    """ 用chatglm生成人物的人设 """
...
{text}
"""
    return get_chatglm_response_via_sdk2(
        messages=[
            {
                "role": "user",
                "content": instruction.strip()
